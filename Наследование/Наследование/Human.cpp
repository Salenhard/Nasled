...
void Human::set_date_of_birth(int new_day, int new_month,int new_year)					// устанавливает дату рождени€
{
...
Human::Human(const std::string& new_name, const std::string& new_secondname, const std::string& new_lastname, int new_day, int new_month, int new_year)		// конструктор с параметрами
{
	set_props(new_name,new_secondname,new_lastname, new_day, new_month, new_year);
}

void Human::set_props(const std::string& new_name, const std::string& new_secondname, const std::string &new_lastname, int new_day, int new_month, int new_year)		// устанвливает все пол€ класса
{
	set_name(new_name);										
